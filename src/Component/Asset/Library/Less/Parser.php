...
use ZimbruCode\Component\Asset\Library\Less\Tree\Variable;
use ZimbruCode\Component\Asset\Library\Less\Tree\Expression;
use ZimbruCode\Component\Asset\Library\Less\SourceMap\Generator;
use ZimbruCode\Component\Asset\Library\Less\Visitor\JoinSelector;
use ZimbruCode\Component\Asset\Library\Less\Visitor\ProcessExtends;
use ZimbruCode\Component\Asset\Library\Less\Visitor\ToCSS;
use ZimbruCode\Component\Asset\Library\Less\Exception\ExceptionParser;
use ZimbruCode\Component\Asset\Library\Less\Exception\ExceptionChunk;
...
use ZimbruCode\Component\Asset\Library\Less\Tree\UnicodeDescriptor;
use ZimbruCode\Component\Asset\Library\Less\Tree\RulesetCall;
use ZimbruCode\Component\Asset\Library\Less\Tree\Selector;
use ZimbruCode\Component\Asset\Library\Less\Tree\Value;
use ZimbruCode\Component\Asset\Library\Less\Tree\Alpha;
use ZimbruCode\Component\Asset\Library\Less\Tree\Paren;
use ZimbruCode\Component\Asset\Library\Less\Tree\DetachedRuleset;
use ZimbruCode\Component\Asset\Library\Less\Tree\Negative;
use ZimbruCode\Component\Asset\Library\Less\Tree\Assignment;
use ZimbruCode\Component\Asset\Library\Less\Tree\Url;
use ZimbruCode\Component\Asset\Library\Less\Tree\Operation;
use ZimbruCode\Component\Asset\Library\Less\Tree\Condition;
use ZimbruCode\Component\Asset\Library\Less\Tree\Javascript;
use ZimbruCode\Component\Asset\Library\Less\Tree\Extend;
use ZimbruCode\Component\Asset\Library\Less\Tree\Attribute;
use ZimbruCode\Component\Asset\Library\Less\Tree\NameValue;
use ZimbruCode\Component\Asset\Library\Less\Tree\Media;
use ZimbruCode\Component\Asset\Library\Less\Tree\Comment;
use ZimbruCode\Component\Asset\Library\Less\Tree\Call;
use ZimbruCode\Component\Asset\Library\Less\Tree\Element;
use ZimbruCode\Component\Asset\Library\Less\Tree\Import;
use ZimbruCode\Component\Asset\Library\Less\Tree\Directive;
...
/**
 * Class : Parsing and compiling less files into css
 *
 * @author  Junjulini
 * @package ZimbruCode
...
class Parser
{
    ...
    public static $default_options = [
        ...
        'relativeUrls'       => true,  // option - whether to adjust URL to be relative
    ...
    public function __construct($env = null)
    {
        ...
        // mbstring.func_overload > 1 bug fix
    ...
    public function getCss()
    {
        ...
        try {
            ...
            if (self::$options['sourceMap']) {
                $generator = new Generator($evaldRoot, self::$contentsMap, self::$options);
                // will also save file
    ...
    private function getVariableValue($var)
    {
        if (!($var instanceof Tree)) {
            throw new Exception('var is not a Tree object');
        }
    ...
    {
        $visitors   = [];
        $visitors[] = new JoinSelector();

        if (self::$has_extends) {
            $visitors[] = new ProcessExtends();
        }
    ...
    {
        $filename = Environment::normalizePath($filename);
    ...
    private function GetRules($file_path)
    {
        ...
        if ($this->pos < $this->input_len) {
            throw new ExceptionChunk($this->input, null, $this->furthest, $this->env->currentFileInfo);
        }
    ...
    private function parseComment()
    {
        ...
        if ($this->input[$this->pos + 1] === '/') {
            ...
            return new Comment($match[0], true, $this->pos, $this->env->currentFileInfo);
        }

        $comment = $this->MatchReg('/\\G\/\*(?s).*?\*+\/\n?/'); // not the same as less.js to prevent fatal errors
        if ($comment) {
            return new Comment($comment[0], false, $this->pos, $this->env->currentFileInfo);
        }
    }
    ...
    private function parseEntitiesQuoted()
    {
        ...
        return new Quoted($quoted, $matched, $e, $index, $this->env->currentFileInfo);
    }
    ...
    // duplicate of Color::FromKeyword
    private function FromKeyword($keyword)
    {
        ...
        if (Colors::hasOwnProperty($keyword)) {
            // detect named color
            return new Color(substr(Colors::color($keyword), 1));
        }
    ...
    private function parseEntitiesCall()
    {
        ...
        if ($name) {
            return new Call($name, $args, $index, $this->env->currentFileInfo);
        }
    }
    ...
    private function parseEntitiesAssignment()
    {
        ...
        if ($value) {
            return new Assignment($key[0], $value);
        }
    }
    ...
    private function parseEntitiesUrl()
    {
        ...
        if (isset($value->value) || $value instanceof Variable) {
            return new Url($value, $this->env->currentFileInfo);
        }

        return new Url(new Anonymous($value), $this->env->currentFileInfo);
    }
    ...
    private function parseEntitiesVariable()
    {
        ...
        if ($this->PeekChar('@') && ($name = $this->MatchReg('/\\G@@?[\w-]+/'))) {
            return new Variable($name[0], $index, $this->env->currentFileInfo);
        }
    }
    ...
    private function parseEntitiesVariableCurly()
    {
        ...
        if ($this->input_len > ($this->pos + 1) && $this->input[$this->pos] === '@' && ($curly = $this->MatchReg('/\\G@\{([\w-]+)\}/'))) {
            return new Variable('@' . $curly[1], $index, $this->env->currentFileInfo);
        }
    }
    ...
    private function parseEntitiesJavascript()
    {
        ...
        $str = $this->MatchReg('/\\G`([^`]*)`/');
        if ($str) {
            return new Javascript($str[1], $this->pos, $e);
        }
    }
    ...
    public function parseExtend($isRule = false)
    {
        ...
        do {
            ...
            $extendList[] = new Extend(new Selector($elements), $option, $index);
    ...
    private function parseMixinCall()
    {
        ...
        if ($elements) {
            ...
            if ($this->parseEnd()) {
                $this->forget();
                return new CallMixin($elements, $args, $index, $this->env->currentFileInfo, $important);
            }
        }
    ...
    private function parseMixinCallElements()
    {
        ...
        while (true) {
            ...
            $elements[] = new Element($c, $e[0], $elemIndex, $this->env->currentFileInfo);
            $c          = $this->MatchChar('>');
        }
    ...
    private function parseMixinDefinition()
    {
        ...
        if ($match) {
            ...
            if (is_array($ruleset)) {
                $this->forget();
                return new DefinitionMixin($name, $params, $ruleset, $cond, $variadic);
            }
    ...
    private function parseElement()
    {
        ...
        if (!is_null($e)) {
            return new Element($c, $e, $index, $this->env->currentFileInfo);
        }
    }
    ...
    private function parseSelector($isLess = false)
    {
        ...
        if ($elements) {
            return new Selector($elements, $extendList, $condition, $index, $this->env->currentFileInfo);
        }
    ...
    private function parseAttribute()
    {
        ...
        $opF = (isset($op[0])) ? $op[0] : null;
        return new Attribute($key, $opF, $val);
    }
    ...
    private function parseRuleset()
    {
        ...
        if ($selectors) {
            ...
            if (is_array($rules)) {
                ...
                return new Ruleset($selectors, $rules); // Environment::$strictImports
            }
        }
    ...
    private function parseNameValue()
    {
        ...
        if ($match) {
            ...
            return new NameValue($match[1], $match[2], $index, $this->env->currentFileInfo);
        }
    ...
    private function parseRule($tryAnonymous = null)
    {
        ...
        if ($name) {
            ...
            if ($value && $this->parseEnd()) {
                $this->forget();
                return new Rule($name, $value, $important, $merge, $startOfRule, $this->env->currentFileInfo);
            } else {
                $this->furthest = $this->pos;
                $this->restore();
    ...
    private function parseImport()
    {
        ...
        if ($dir) {
            ...
            if ($path) {
                ...
                if ($this->MatchChar(';')) {
                    if ($features) {
                        $features = new Value($features);
                    }

                    $this->forget();
                    return new Import($path, $features, $options, $this->pos, $this->env->currentFileInfo);
                }
            }
        }
    ...
    private function parseMediaFeature()
    {
        ...
        do {
            ...
            if ($e) {
                ...
                if ($this->MatchChar(')')) {
                    if ($p && $e) {
                        $r       = new Rule($p, $e, null, null, $this->pos, $this->env->currentFileInfo, true);
                        $nodes[] = new Paren($r);
                    } elseif ($e) {
                        $nodes[] = new Paren($e);
                    } else {
    ...
    private function parseMedia()
    {
        if ($this->MatchReg('/\\G@media/')) {
            ...
            if (is_array($rules)) {
                return new Media($rules, $features, $this->pos, $this->env->currentFileInfo);
            }
        }
    }
    ...
    private function parseDirective()
    {
        ...
        if ($hasIdentifier) {
            ...
            if ($value) {
                $value = new Anonymous(trim($value[0]));
            }
        }
        ...
        if ($rules || (!$hasBlock && $value && $this->MatchChar(';'))) {
            $this->forget();
            return new Directive($name, $value, $rules, $index, $this->env->currentFileInfo);
        }
    ...
    private function parseSub()
    {
        if ($this->MatchChar('(')) {
            ...
            if ($a) {
                $this->expectChar(')');
                return new Expression([$a], true); //instead of $e->parens = true so the value is cached
            }
        }
    }
    ...
    public function parseMultiplication()
    {
        ...
        if ($return) {
            while (true) {
                ...
                $m->parensInOp = true;
                $a->parensInOp = true;
                $return        = new Operation($op, [$return, $a], $isSpaced);
            }
        }
    ...
    private function parseConditions()
    {
        ...
        if ($a) {
            while (true) {
                ...
                $return = new Condition('or', $return, $b, $index);
            }
    ...
    private function parseCondition()
    {
        ...
        if ($a) {
            ...
            if ($op) {
                ...
                if ($b) {
                    $c = new Condition($op[0], $a, $b, $index, $negate);
                } else {
                    $this->Error('Unexpected expression');
                }
            } else {
                $k = new Keyword('true');
                $c = new Condition('=', $a, $k, $index, $negate);
            }
            $this->expectChar(')');

            return $this->MatchReg('/\\Gand/') ? new Condition('and', $c, $this->parseCondition()) : $c;
        }
    }
    ...
    private function parseRuleProperty()
    {
        ...
        if ((count($name) > 1) && $this->rulePropertyMatch('/\\G\s*((?:\+_|\+)?)\s*:/', $offset, $length, $index, $name)) {
            ...
            foreach ($name as $k => $s) {
                if (!$s || $s[0] !== '@') {
                    $name[$k] = new Keyword($s);
                } else {
                    $name[$k] = new Variable('@' . substr($s, 2, -1), $index[$k], $this->env->currentFileInfo);
                }
            }
    ...
    {
        throw new ExceptionParser($msg, null, $this->furthest, $this->env->currentFileInfo);
    }
