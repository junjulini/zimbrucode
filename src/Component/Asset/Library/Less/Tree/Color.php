...
use ZimbruCode\Component\Asset\Library\Less\Colors;
...
class Color extends Tree
{
    ...
    {
        $compress = Parser::$options['compress'] && !$doNotCompress;
        $alpha    = Functions::fround($this->alpha);
        ...
        if ($alpha < 1) {
            ...
            foreach ($this->rgb as $c) {
                $values[] = Functions::clamp(round($c), 255);
            }
            $values[] = $alpha;
    ...
    public function operate($op, $other)
    {
        ...
        for ($c = 0; $c < 3; $c++) {
            $rgb[$c] = Functions::operate($op, $this->rgb[$c], $other->rgb[$c]);
        }
    ...
    {
        $argb = array_merge((array) Parser::round($this->alpha * 255), $this->rgb);
    ...
    public function toHex($v)
    {
        $ret = '#';
        foreach ($v as $c) {
            $c = Functions::clamp(Parser::round($c), 255);
            if ($c < 16) {
                $ret .= '0';
            }
            $ret .= dechex($c);
