...
class Call extends Tree
{
    ...
    public function compile($env = null)
    {
        ...
        $result = null;
        if ($nameLC === 'default') {
            $result = DefaultFunc::compile();

        } else {

            if (method_exists('ZimbruCode\\Component\\Asset\\Library\\Less\\Functions', $nameLC)) {
                // 1.
                try {

                    $func   = new Functions($env, $this->currentFileInfo);
                    $result = call_user_func_array([$func, $nameLC], $args);

                } catch (Exception $e) {
                    throw new ExceptionCompiler('error evaluating function `' . $this->name . '` ' . $e->getMessage() . ' index: ' . $this->index);
                }
