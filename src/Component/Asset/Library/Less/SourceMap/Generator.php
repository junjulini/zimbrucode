...
namespace ZimbruCode\Component\Asset\Library\Less\SourceMap;

use ZimbruCode\Component\Asset\Library\Less\Configurable;
use ZimbruCode\Component\Asset\Library\Less\Functions;
use ZimbruCode\Component\Asset\Library\Less\Output\OutputMapped;
use ZimbruCode\Component\Asset\Library\Less\Tree\Ruleset;

/**
 * Class : Source map generator
 *
 * @author  Junjulini
 * @package ZimbruCode
...
class Generator extends Configurable
{
     ...
     * Constructor
     *
     * @param Ruleset $root The root node
     * @param array $options Array of options
     */
    public function __construct(Ruleset $root, $contentsMap, $options = [])
    {
        $this->root        = $root;
    ...
    {
        $output = new OutputMapped($this->contentsMap, $this);
        ...
        // inline the map
        if (!$sourceMapUrl) {
            $sourceMapUrl = sprintf('data:application/json,%s', Functions::encodeURIComponent($sourceMapContent));
        }
